...
def count_ced(predicted_points, gt_points, args, normalization):
        ...
        for img_name in predicted_points[method_name].keys():
            if img_name in gt_points:
                ...
                if args.normalization_type == 'eyes':
                    ...
                    normalization_factor = normalization[img_name]
                else:
...
def main():
    ...
    normalization = {}
    with open(args.normalization_path) as f:
        for line in f:
            file, norm = line.split()
            normalization[file] = float(norm)
    ...
    predicted_points1 = {}
    if args.predictions_path1:
        for pred_path in args.predictions_path1:
            predicted_points1[os.path.basename(pred_path)] = read_points(pred_path, args.max_points_to_read)

    gt_points = read_points(args.gt_path, args.max_points_to_read)
    # print(predicted_points.keys())
    # print(gt_points)

    ceds = count_ced(predicted_points, gt_points, args, normalization)
    if predicted_points1:
        ceds1 = count_ced(predicted_points1, gt_points, args, normalization)
    else:
        ceds1 = None
    ...
    for method_idx, method_name in enumerate(ceds.keys()):
        ...
        plt.plot(err[under_thr_range], proportion[under_thr_range], label=args.predictions_path[0].split('/')[-1] + ', auc={:1.3f}'.format(cur_auc),
                 linestyle=line_styles[method_idx % len(line_styles)], linewidth=2.0)
    if ceds1:
        for method_idx, method_name in enumerate(ceds1.keys()):
            print('Plotting graph for the method {}'.format(method_name))
            err = ceds1[method_name]
            proportion = np.arange(err.shape[0], dtype=np.float32) / err.shape[0]
            under_thr = err > args.error_thr
            last_idx = len(err)
            if len(np.flatnonzero(under_thr)) > 0:
            ...
            plt.plot(err[under_thr_range], proportion[under_thr_range], label=args.predictions_path1[0].split('/')[-1] + ', auc={:1.3f}'.format(cur_auc),
                     linestyle=line_styles[method_idx % len(line_styles)], linewidth=2.0)
